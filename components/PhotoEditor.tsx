...
import { CloseIcon, DownloadIcon, SaveIcon, RedrawIcon, MirrorIcon, FlipVerticalIcon, CropIcon, ZoomInIcon, ZoomOutIcon, ArrowsPointingOutIcon, SwapVerticalIcon, TextIcon, TrashIcon, RotateIcon, ImageIcon, LightBrushIcon, UndoIcon, SharpenIcon, BlurIcon, EraserIcon, EyeIcon, BrushIcon, PlusIcon, EyeSlashIcon, SunIcon, MoonIcon, MagicEraserIcon, SparklesIcon, SaturationIcon, ContrastIcon, FilmIcon, UserCircleIcon, CompareIcon, LeafIcon, ArrowUpIcon, ArrowDownIcon, ArrowLeftIcon, ArrowRightIcon, SplitViewIcon, RadialGradientIcon, LinearGradientIcon, InvertIcon } from './Icons';
    ...
    ({ label, value, onChange, min = -100, max = 100, step = 1, resetValue }) => (
        <div className="space-y-1.5">
            <div className="flex justify-between items-center">
                <label onDoubleClick={() => onChange(resetValue)} className="text-[15px] text-gray-300 font-bold cursor-pointer uppercase tracking-tight" title="Double click to reset">{label}</label>
                <span onDoubleClick={() => onChange(resetValue)} className="text-sm text-gray-200 font-mono bg-gray-900 border border-white/10 px-2.5 py-0.5 rounded cursor-pointer" title="Double click to reset">{value}</span>
            </div>
            <input
                type="range"
                min={min}
...
const AdjustmentBrushPanel: React.FC<{
    ...
    onMaskLayersChange: (action: React.SetStateAction<MaskLayer[]>) => void;
    ...
    const handleDeleteLayer = (id: string) => {
        ...
        onMaskLayersChange(() => newLayers); // 強制更新為過濾後的列表

        // 邏輯修正：如果刪除的是正在使用的圖層，則自動選取其他圖層
        if (activeMaskLayerId === id) {
            if (newLayers.length > 0) {
                onActiveMaskLayerIdChange(newLayers[Math.max(0, index - 1)].id);
            } else {
                onActiveMaskLayerIdChange(null);
            }
        }
    };

    const handleUpdateLayer = (id: string, updates: Partial<MaskLayer>) => {
        onMaskLayersChange(prev => prev.map(l => l.id === id ? { ...l, ...updates } : l));
    };
    ...
    return (
        <div className="space-y-5">
             ...
             <div className="p-4 bg-gray-900/50 rounded-xl space-y-4 border border-white/5 shadow-sm">
                <div className="flex justify-between items-center mb-1">
                    <h4 className="text-sm font-black text-gray-300 uppercase tracking-widest">{t('maskLayersLabel')}</h4>
                    <div className="flex gap-2">
                        <button onClick={() => handleAddLayer('brush')} className="p-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg text-gray-200 transition-colors shadow-sm" title={t('addBrushMaskButton')}><BrushIcon className="w-5 h-5"/></button>
                        <button onClick={() => handleAddLayer('radial')} className="p-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg text-gray-200 transition-colors shadow-sm" title={t('addRadialMaskButton')}><RadialGradientIcon className="w-5 h-5"/></button>
                        <button onClick={() => handleAddLayer('linear')} className="p-2.5 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg text-gray-200 transition-colors shadow-sm" title={t('addLinearMaskButton')}><LinearGradientIcon className="w-5 h-5"/></button>
                    </div>
                </div>
                <div className="space-y-2 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                    {maskLayers.map(layer => (
                        <div key={layer.id} 
                             className={`flex items-center justify-between p-3 rounded-xl cursor-pointer border transition-all duration-200 ${layer.id === activeMaskLayerId ? 'bg-purple-900/40 border-purple-500/60 ring-1 ring-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.1)]' : 'bg-gray-800/60 border-transparent text-gray-400 hover:bg-gray-700/80 hover:text-gray-200'}`}
                             onClick={() => onActiveMaskLayerIdChange(layer.id)}
                        >
                            <div className="flex items-center gap-3 overflow-hidden">
                                <div className={`${layer.id === activeMaskLayerId ? 'text-purple-400' : 'text-gray-500'}`}>
                                    {layer.type === 'brush' && <BrushIcon className="w-5 h-5 flex-shrink-0" />}
                                    {layer.type === 'radial' && <RadialGradientIcon className="w-5 h-5 flex-shrink-0" />}
                                    {layer.type === 'linear' && <LinearGradientIcon className="w-5 h-5 flex-shrink-0" />}
                                </div>
                                <span className={`text-[14px] font-bold truncate ${layer.id === activeMaskLayerId ? 'text-white' : ''} ${!layer.isVisible ? 'opacity-40' : ''}`}>{layer.name}</span>
                            </div>
                            <div className="flex items-center gap-1.5 ml-2">
                                <button 
                                    onClick={(e) => { e.stopPropagation(); handleUpdateLayer(layer.id, { isVisible: !layer.isVisible }); }} 
                                    className={`p-2 rounded-lg transition-all border ${layer.isVisible ? 'text-white bg-green-500 border-green-400 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'text-gray-500 bg-gray-900 border-gray-700 opacity-60'}`}
                                    title={layer.isVisible ? t('showMaskLabel') : t('invertMaskLabel')}
                                >
                                ...
                                <button 
                                    ...
                                    className="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-red-600 border border-transparent hover:border-red-400 transition-all"
             ...
             {activeLayer && (
                <>
                    <div className="p-4 bg-gray-900/50 rounded-xl border border-white/5 space-y-5 shadow-sm">
                        <div className="flex justify-between items-center">
                            <h4 className="text-sm font-black text-gray-300 uppercase tracking-widest">{t('adjustmentBrushLabel')}</h4>
                             <div className="flex items-center gap-4">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" checked={activeLayer.invert} onChange={(e) => handleUpdateLayer(activeLayer.id, { invert: e.target.checked })} className="w-5 h-5 rounded bg-gray-800 border-gray-600 text-purple-600 focus:ring-purple-500 shadow-sm" />
                                    <span className="text-[12px] font-bold text-gray-400">{t('invertMaskLabel')}</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" checked={settings.showMask} onChange={(e) => onSettingsChange(p => ({ ...p, showMask: e.target.checked }))} className="w-5 h-5 rounded bg-gray-800 border-gray-600 text-purple-600 focus:ring-purple-500 shadow-sm" />
                                    <span className="text-[12px] font-bold text-gray-400">{t('showMaskLabel')}</span>
                                </label>
                             </div>
                        </div>
                        ...
                        {activeLayer.type === 'brush' && (
                            <>
                                <div className="flex bg-gray-800 rounded-xl p-0.5 border border-gray-700 shadow-inner">
                                    <button onClick={() => onSettingsChange(s => ({ ...s, isErasing: false }))} className={`flex-1 py-2 text-sm font-black uppercase rounded-lg ${!settings.isErasing ? 'bg-purple-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}>{t('paintButton')}</button>
                                    <button onClick={() => onSettingsChange(s => ({ ...s, isErasing: true }))} className={`flex-1 py-2 text-sm font-black uppercase rounded-lg ${settings.isErasing ? 'bg-purple-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-300'}`}>{t('eraseButton')}</button>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-black text-gray-400 uppercase tracking-widest">{t('brushSizeLabel')}</span>
                                    <button onClick={onUndo} className="text-sm text-purple-400 hover:text-purple-300 font-bold flex items-center gap-1.5"><UndoIcon className="w-4 h-4"/> {t('undoButton')}</button>
                                </div>
                                <AdjustmentSlider label="" value={settings.size} min={1} max={300} onChange={v => onSettingsChange(s => ({ ...s, size: v }))} resetValue={50} />
                                
                                <span className="text-sm font-black text-gray-400 uppercase tracking-widest">{t('brushFeatherLabel')}</span>
                                <AdjustmentSlider label="" value={settings.feather} min={0} max={100} onChange={v => onSettingsChange(s => ({ ...s, feather: v }))} resetValue={80} />
                                
                                <span className="text-sm font-black text-gray-400 uppercase tracking-widest">{t('brushStrengthLabel')}</span>
                        ...
                        {(activeLayer.type === 'linear' || activeLayer.type === 'radial') && activeLayer.gradient && (
                             <>
                                <h4 className="text-sm font-black text-gray-300 uppercase tracking-widest">{t('gradientSettingsLabel')}</h4>
                                <span className="text-sm font-black text-gray-400 uppercase tracking-widest">{t('brushFeatherLabel')}</span>
                    ...
                    <div className="space-y-5">
                         <div className="p-4 bg-gray-900/50 rounded-xl border border-white/5 space-y-5 shadow-sm">
                            <h4 className="text-sm font-black text-gray-300 uppercase tracking-widest border-b border-white/5 pb-1.5">{t('lightLabel')}</h4>
                         ...
                         <div className="p-4 bg-gray-900/50 rounded-xl border border-white/5 space-y-5 shadow-sm">
                            <h4 className="text-sm font-black text-gray-300 uppercase tracking-widest border-b border-white/5 pb-1.5">{t('colorLabel')}</h4>
                         ...
                         <div className="p-4 bg-gray-900/50 rounded-xl border border-white/5 space-y-5 shadow-sm">
                             <h4 className="text-sm font-black text-gray-300 uppercase tracking-widest border-b border-white/5 pb-1.5">{t('clarityLabel')}</h4>
                            ...
                            <AdjustmentSlider label={t('dehazeLabel')} value={activeLayer.adjustments.dehaze} min={0} max={100} onChange={v => setLayerAdjustments(p => ({ ...p, dehaze: v }))} resetValue={0} />
                            <AdjustmentSlider label={t('blurLabel')} value={activeLayer.adjustments.blur} min={0} max={20} onChange={v => setLayerAdjustments(p => ({ ...p, blur: v }))} resetValue={0} />
                         </div>
                    </div>
                </>
             )}
...
export const PhotoEditor: React.FC<PhotoEditorProps> = ({ image, onSave, onClose, t, userCredits, onDeductCredits }) => {
    ...
    const [activeTab, setActiveTab] = useState<'adjust' | 'lightBrush' | 'adjustmentBrush' | 'text'>('adjust');
    ...
    const panStartRef = useRef({ startX: 0, startY: 0, startPan: pan });
    ...
    const handleDarkenEffectClick = useCallback(() => {
        setAdjustments(prev => ({
            ...prev,
            highlights: Math.max(-100, prev.highlights - 15),
            exposure: Math.max(-100, prev.exposure - 5),
            brightness: Math.max(-100, prev.brightness - 3),
            contrast: Math.min(100, prev.contrast + 5)
        }));
    }, []);

    const hasInitializedMaskRef = useRef(false);
    useEffect(() => {
        // 修正處：僅在「從未初始化」的情況下才建立第一個遮罩。
        // 這能確保使用者刪除到 0 時，不會再被自動產生的補位邏輯干擾。
        if (activeTab === 'adjustmentBrush' && !hasInitializedMaskRef.current) {
            ...
            hasInitializedMaskRef.current = true;
        }
    }, [activeTab, t]);
    ...
    const resetAll = useCallback(() => {
        ...
        hasInitializedMaskRef.current = false;
    }, []);
    ...
    const fitImageToScreen = resetView;
        ...
        maskLayers.forEach(layer => {
            ...
            if (layer.type === 'brush') {
                 ...
                 if (layer.type === 'linear') {
                     const { start, end } = layer.gradient;
    ...
    useEffect(() => {
        ...
        ctx.globalCompositeOperation = 'source-over';
    }, [isCropping, sourceImage, adjustments, maskLayers, activeMaskLayerId, adjustmentBrushSettings.showMask, lightBrushStrokes, redrawTrigger, activeTab, isComparing]);
    ...
    const handlePanStart = useCallback((clientX: number, clientY: number, force = false) => {
        if(!force && (isCropping || selectedOverlayId || (activeTab === 'adjustmentBrush' && activeMaskLayer?.type === 'brush') || activeTab === 'lightBrush' || isGradientInteractRef.current)) return;
    ...
    const getCoordsInImage = useCallback((clientX: number, clientY: number, clamp: boolean = true) => {
        ...
        if (clamp && (relativeX < 0 || relativeX > displayDims.width || relativeX > displayDims.height)) return null;
    ...
    const drawBrushPreview = useCallback((clientX: number, clientY: number) => {
        ...
        if (!canvas || ((activeTab !== 'adjustmentBrush' || activeMaskLayer?.type !== 'brush') && activeTab !== 'lightBrush') || !sourceImage) return;
        ...
        ctx.stroke();
        ctx.setLineDash([]);
    }, [activeTab, sourceImage, getCoordsInImage, adjustmentBrushSettings, lightBrushSettings, activeMaskLayer]);
    ...
    }, [activeTab, getCoordsInImage, adjustmentBrushSettings, lightBrushSettings, handlePanStart, isSpacebarDown, activeMaskLayerId, isSplitView, displayDims, zoom, pan, activeMaskLayer?.type]);
    
    const onMouseMove = useCallback((e: React.MouseEvent) => {
        e.preventDefault();
        const isBrushActive = (activeTab === 'adjustmentBrush' && activeMaskLayer?.type === 'brush') || activeTab === 'lightBrush';
    ...
    const onTouchStart = (e: React.TouchEvent) => {
        ...
        if (e.touches.length === 2) {
            ...
            const isBrushTab = (activeTab === 'adjustmentBrush' && activeMaskLayer?.type === 'brush') || activeTab === 'lightBrush';
    ...
    if (isCropping) {
        ...
        cursorStyle = 'grab';
    } else if ((activeTab === 'adjustmentBrush' && activeMaskLayer?.type === 'brush') || activeTab === 'lightBrush') {
        cursorStyle = 'none';
    } else {
        cursorStyle = 'grab';
    }
            ...
            <main className="flex-grow flex flex-col lg:flex-row gap-4 overflow-hidden">
                <div 
                    ...
                    onClick={() => { 
                        ...
                        if(!isCropping && (activeTab !== 'adjustmentBrush' || activeMaskLayer?.type !== 'brush') && activeTab !== 'lightBrush' && !isSpacebarDown && !isSplitView) setSelectedOverlayId(null); 
                    }}
                >
                ...
                {!isCropping &&
                    <div className="w-full lg:w-[400px] h-1/2 lg:h-auto flex-shrink-0 bg-gray-800 rounded-lg p-6 flex flex-col gap-6 overflow-y-auto custom-scrollbar shadow-2xl border-l border-white/5">
                        <div className="flex justify-between items-center mb-1">
                            <h3 className="text-sm font-black text-gray-200 uppercase tracking-widest">{t('adjustmentsLabel')}</h3>
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => setIsSplitView(!isSplitView)}
                                    className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-[13px] font-bold uppercase transition-all ${isSplitView ? 'bg-blue-600/20 text-blue-400 border-blue-500/50' : 'bg-gray-900/50 text-gray-400 border-gray-700 hover:text-gray-200'}`}
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-gray-700 bg-gray-900/50 text-gray-400 hover:text-gray-200 text-[13px] font-bold uppercase"
                                    title={t('compareButton')}
                                >
                                    <CompareIcon className="w-4 h-4" />
                                    <span>{t('compareButton')}</span>
                                </button>
                                <button onClick={() => {resetAll(); resetView();}} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-purple-500/30 bg-purple-900/10 text-purple-400 hover:text-purple-300 text-[13px] font-bold uppercase">
                                    <RedrawIcon className="w-4 h-4"/>{t('resetButton')}
                                </button>
                            </div>
                        </div>
                        
                         <div className="text-sm text-gray-300 bg-gray-900/50 p-4 rounded-xl border border-white/5 space-y-2">
                            <div className="flex justify-between items-center">
                                <span className="uppercase tracking-tighter font-bold text-gray-400">{t('resolutionLabel')}</span>
                                <span className="font-mono text-gray-200">{imageInfo.width} x {imageInfo.height}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="uppercase tracking-tighter font-bold text-gray-400">{t('sizeLabel')}</span>
                                <span className="font-mono text-gray-200">{formatFileSize(imageInfo.size)}</span>
                            </div>
                        </div>

                         <div className="flex items-center justify-center gap-3 bg-gray-900/50 p-2.5 rounded-xl border border-white/5">
                            <button onClick={() => setZoom(z => Math.max(1, z - 0.2))} className="p-2.5 rounded-xl hover:bg-gray-700 border border-gray-800 text-gray-400 transition-colors shadow-sm" title={t('zoomOutButton')}><ZoomOutIcon className="w-6 h-6"/></button>
                            <span className="text-sm font-mono w-20 text-center bg-gray-800 text-gray-100 rounded-xl border border-white/5 py-2 shadow-inner" onDoubleClick={fitImageToScreen}>{Math.round(zoom * 100)}%</span>
                            <button onClick={() => setZoom(z => Math.min(3, z + 0.2))} className="p-2.5 rounded-xl hover:bg-gray-700 border border-gray-800 text-gray-400 transition-colors shadow-sm" title={t('zoomInButton')}><ZoomInIcon className="w-6 h-6"/></button>
                            <button onClick={fitImageToScreen} className="p-2.5 rounded-xl hover:bg-gray-700 border border-gray-800 text-gray-400 transition-colors shadow-sm" title={t('resetViewButton')}><ArrowsPointingOutIcon className="w-6 h-6"/></button>
                        </div>
                        
                        <div className="flex bg-gray-900/80 rounded-2xl p-1 border border-white/5 shadow-2xl">
                            <button onClick={() => setActiveTab('adjust')} className={`flex-1 py-2.5 text-[15px] font-black uppercase tracking-tighter rounded-xl transition-all duration-300 ${activeTab === 'adjust' ? 'bg-purple-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'text-gray-400 hover:text-gray-200'}`}>{t('adjustmentsLabel')}</button>
                            <button onClick={() => setActiveTab('lightBrush')} className={`flex-1 py-2.5 text-[15px] font-black uppercase tracking-tighter rounded-xl transition-all duration-300 ${activeTab === 'lightBrush' ? 'bg-purple-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'text-gray-400 hover:text-gray-200'}`}>{t('lightbrushLabel')}</button>
                            <button onClick={() => setActiveTab('adjustmentBrush')} className={`flex-1 py-2.5 text-[15px] font-black uppercase tracking-tighter rounded-xl transition-all duration-300 ${activeTab === 'adjustmentBrush' ? 'bg-purple-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'text-gray-400 hover:text-gray-200'}`}>{t('adjustmentBrushLabel')}</button>
                            <button onClick={() => setActiveTab('text')} className={`flex-1 py-2.5 text-[15px] font-black uppercase tracking-tighter rounded-xl transition-all duration-300 ${activeTab === 'text' ? 'bg-purple-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'text-gray-400 hover:text-gray-200'}`}>{t('textLabel')}</button>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <AdjustmentPanel
                                    ...
                                    onDarkenEffect={handleDarkenEffectClick}
                                />
                            )}
...
const AdjustmentPanel: React.FC<AdjustmentPanelProps> = ({ adjustments, setAdjustments, onFlip, onRotate, onCrop, t, onBrightenEffect, onDarkenEffect }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-5 shadow-sm">
                <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest">{t('enhanceLabel')}</h4>
                 <AdjustmentSlider 
                    label={t('accentLabel')} 
            ...
            <div className="grid grid-cols-2 gap-4">
                <button
                    ...
                    className="w-full flex items-center justify-center gap-2.5 bg-gradient-to-r from-yellow-500 to-orange-500 text-white font-black text-[15px] py-3 px-2 rounded-2xl border border-white/20 hover:brightness-110 transition-all shadow-xl transform active:scale-95 uppercase tracking-tighter"
                >
                    <SunIcon className="w-5 h-5" />
                    <span>{t('brightenEffectButton')}</span>
                </button>
                <button
                    onClick={onDarkenEffect}
                    className="w-full flex items-center justify-center gap-2.5 bg-gradient-to-r from-indigo-500 to-blue-700 text-white font-black text-[15px] py-3 px-2 rounded-2xl border border-white/20 hover:brightness-110 transition-all shadow-xl transform active:scale-95 uppercase tracking-tighter"
                >
                    <MoonIcon className="w-5 h-5" />
                    <span>{t('aiAutoFixButton')}</span>
                </button>
            </div>
            
            <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-4 shadow-sm">
                <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest">{t('quickEffectsLabel')}</h4>
                <div className="grid grid-cols-3 gap-4">
                    {effects.map(effect => (
                        <button
                            key={effect.id}
                            onClick={() => applyEffect(effect.adjustments)}
                            className="flex flex-col items-center justify-center gap-2 p-2.5 bg-gray-800 hover:bg-gray-700 text-white rounded-2xl border border-gray-700 transition-all aspect-square text-center shadow-md group"
                            title={effect.label}
                        >
                            <effect.icon className="w-6 h-6 text-purple-400 group-hover:scale-110 transition-transform" />
                            <span className="text-[13px] font-black leading-tight uppercase tracking-tighter mt-1">{effect.label}</span>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-5 shadow-sm">
                <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest border-b border-white/5 pb-2">{t('lightLabel')}</h4>
            ...
            <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-5 shadow-sm">
                <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest border-b border-white/5 pb-2">{t('colorLabel')}</h4>
             ...
             <ColorMixerPanel
                ...
                onReset={() => {
                    setAdjustments(prev => ({ ...prev, colorMixer: INITIAL_ADJUSTMENTS.colorMixer }))
                }}
                t={t}
            />
            <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-5 shadow-sm">
                <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest border-b border-white/5 pb-2">{t('clarityLabel')}</h4>
            ...
            <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-5 shadow-sm">
                 <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest border-b border-white/5 pb-2">{t('transformLabel')}</h4>
                <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => onFlip('X')} className="flex items-center justify-center gap-2.5 bg-gray-800 hover:bg-gray-700 text-gray-200 text-[15px] font-black py-3 rounded-2xl border border-gray-700 transition-all uppercase tracking-tighter shadow-md"><MirrorIcon className="w-5 h-5"/> {t('mirrorButton')}</button>
                    <button onClick={() => onFlip('Y')} className="flex items-center justify-center gap-2.5 bg-gray-800 hover:bg-gray-700 text-gray-200 text-[15px] font-black py-3 rounded-2xl border border-gray-700 transition-all uppercase tracking-tighter shadow-md"><FlipVerticalIcon className="w-5 h-5"/> {t('flipVerticalButton')}</button>
                </div>
                 <button onClick={onCrop} className="w-full flex items-center justify-center gap-2.5 bg-gray-800 hover:bg-gray-700 text-gray-200 text-[15px] font-black py-3 rounded-2xl border border-gray-700 transition-all uppercase tracking-tighter shadow-md"><CropIcon className="w-5 h-5"/> {t('cropLabel')}</button>
            </div>
        </div>
    );
};
...
const CropUI: React.FC<CropUIProps> = ({ image, onApply, onCancel, t }) => {
    ...
    const handleInteractionMove = useCallback((e: MouseEvent | TouchEvent) => {
        ...
        if (interaction.type === 'move') {
            ...
            if (handles.includes('r')) dw = rotatedDx; if (handles.includes('l')) dw = -rotatedDx;
            if (handles.includes('b')) dh = rotatedDy; if (handles.includes('t')) dh = -rotatedDy;
            const ar = aspectRatios[aspectRatioKey].value;
            if (ar) {
                if (handles.length === 1) { 
    ...
    const cropBoxStyle: React.CSSProperties = {
        left: `${crop.cx - crop.width / 2}%`, top: `${crop.cy - crop.height / 2}%`,
        width: `${crop.width}%`, height: `${crop.height}%`, transform: `rotate(${crop.rotation}deg)`,
    };
            ...
            <div ref={containerRef} className="relative w-full h-full max-w-[90vw] max-h-[calc(100vh-150px)] flex items-center justify-center p-4">
                <div className="relative" style={{ width: imageDims.width, height: imageDims.height }}>
                    ...
                    <div className="absolute inset-0">
                        <div className="absolute cursor-move" style={{ ...cropBoxStyle, boxShadow: '0 0 0 9999px rgba(0,0,0,0.5)' }} onMouseDown={(e) => handleInteractionStart(e, 'move', 'move')} onTouchStart={(e) => handleInteractionStart(e, 'move', 'move')}>
                            ...
                            {HANDLES.map(handle => (
                                <div key={handle} className={`absolute w-3 h-3 bg-white rounded-full 
                                    ...
                                    style={{ transform: `translate(-50%, -50%) rotate(${-crop.rotation}deg)`, cursor: (handle === 'tl' || handle === 'br') ? 'nwse-resize' : (handle === 'tr' || handle === 'bl') ? 'nesw-resize' : (handle === 't' || handle === 'b') ? 'ns-resize' : 'ew-resize' }}
                                    onMouseDown={(e) => handleInteractionStart(e, 'resize', handle)} onTouchStart={(e) => handleInteractionStart(e, 'resize', handle)}
                                />
                            ))}
                            <div className="absolute left-1/2 -bottom-8 w-6 h-6 bg-purple-500 rounded-full border-2 border-white cursor-grab flex items-center justify-center" style={{ transform: `translateX(-50%) rotate(${-crop.rotation}deg)`}} onMouseDown={(e) => handleInteractionStart(e, 'rotate', 'rotate')} onTouchStart={(e) => handleInteractionStart(e, 'rotate', 'rotate')}>
...
const TextPanel: React.FC<{
    ...
    const positions = [
        { name: 'top-left', x: (ov: Overlay) => ov.width / 2, y: (ov: Overlay) => ov.height / 2 },
        { name: 'top-center', x: () => 50, y: (ov: Overlay) => ov.height / 2 },
        { name: 'top-right', x: (ov: Overlay) => 100 - ov.width / 2, y: (ov: Overlay) => ov.height / 2 },
        { name: 'middle-left', x: (ov: Overlay) => ov.width / 2, y: () => 50 },
        { name: 'center', x: () => 50, y: () => 50 },
        { name: 'middle-right', x: (ov: Overlay) => 100 - ov.width / 2, y: () => 50 },
        { name: 'bottom-left', x: (ov: Overlay) => ov.width / 2, y: (ov: Overlay) => 100 - ov.height / 2 },
        { name: 'bottom-center', x: () => 50, y: (ov: Overlay) => 100 - ov.height / 2 },
        { name: 'bottom-right', x: (ov: Overlay) => 100 - ov.width / 2, y: (ov: Overlay) => 100 - ov.height / 2 },
    ];

    const handlePositionClick = (xCalc: (ov: Overlay) => number, yCalc: (ov: Overlay) => number) => {
        if (selectedOverlay) {
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
                <button onClick={onAddText} className="flex items-center justify-center gap-2.5 w-full bg-purple-600 hover:bg-purple-700 text-white font-black py-4 px-3 rounded-2xl border border-white/10 shadow-xl transform active:scale-95 text-[15px] uppercase tracking-tighter transition-all duration-300"><TextIcon className="w-6 h-6"/>{t('addTextButton')}</button>
                <button onClick={onAddWatermark} className="flex items-center justify-center gap-2.5 w-full bg-gray-800 hover:bg-gray-700 text-gray-200 font-black py-4 px-3 rounded-2xl border border-gray-700 shadow-xl transform active:scale-95 text-[15px] uppercase tracking-tighter transition-all duration-300"><ImageIcon className="w-6 h-6"/>{t('addWatermarkButton')}</button>
            </div>
            
            <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-4 shadow-inner">
                 <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest">{t('textTemplatesTitle')}</h4>
                 <div className="grid grid-cols-2 gap-4 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                    {templates.map(template => (
                        <button key={template.id} onClick={() => onAddFromTemplate(template)} className="text-[14px] font-bold text-left bg-gray-800 hover:bg-gray-700 text-gray-300 py-3 px-3 rounded-xl border border-gray-700 truncate shadow-sm transition-colors">{template.text}</button>
                    ))}
                 </div>
            </div>

            {selectedOverlay && (
                <div className="space-y-6 p-5 bg-gray-900/50 rounded-2xl border border-purple-500/50 shadow-[0_0_25px_rgba(168,85,247,0.15)]">
                    {selectedOverlay.type === 'text' && (
                        <>
                            <div>
                                <label htmlFor="textContent" className="text-[15px] font-black text-gray-300 uppercase tracking-widest mb-2.5 block">{t('textInputLabel')}</label>
                                <textarea
                                    id="textContent"
                                    value={selectedOverlay.text}
                                    onChange={(e) => handleStyleChange('text', e.target.value)}
                                    className="w-full h-24 p-4 bg-gray-900 border border-gray-700 rounded-xl text-base text-gray-100 focus:border-purple-500 outline-none transition-all custom-scrollbar shadow-inner"
                                />
                            </div>
                            <div>
                                <label htmlFor="fontFamily" className="text-[15px] font-black text-gray-300 uppercase tracking-widest mb-2.5 block">{t('fontLabel')}</label>
                                <select
                                    id="fontFamily"
                                    value={selectedOverlay.fontFamily}
                                    onChange={(e) => handleStyleChange('fontFamily', e.target.value)}
                                    className="w-full p-3 bg-gray-800 border border-gray-700 rounded-xl text-sm text-gray-200 focus:border-purple-500 outline-none cursor-pointer shadow-sm"
                                >
                             ...
                             <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[15px] font-black text-gray-300 uppercase tracking-widest mb-2.5 block">{t('colorLabel')}</label>
                                    <input
                                        type="color"
                                        value={selectedOverlay.color}
                                        onChange={(e) => handleStyleChange('color', e.target.value)}
                                        className="w-full h-12 rounded-xl border border-gray-700 bg-gray-800 cursor-pointer p-1 overflow-hidden shadow-sm"
                                    />
                                </div>
                                <div className="flex gap-2.5 items-end">
                                    <button onClick={() => handleStyleChange('bold', !selectedOverlay.bold)} className={`flex-1 h-12 rounded-xl border border-gray-700 text-sm font-black transition-all ${selectedOverlay.bold ? 'bg-purple-600 text-white border-purple-500 shadow-md' : 'bg-gray-800 text-gray-400'}`}>B</button>
                                    <button onClick={() => handleStyleChange('italic', !selectedOverlay.italic)} className={`flex-1 h-12 rounded-xl border border-gray-700 text-sm font-black italic transition-all ${selectedOverlay.italic ? 'bg-purple-600 text-white border-purple-500 shadow-md' : 'bg-gray-800 text-gray-400'}`}>I</button>
                                </div>
                             </div>
                    ...
                    <div>
                         <label className="text-[15px] font-black text-gray-300 uppercase tracking-widest mb-2.5 block">{t('positionLabel')}</label>
                         <div className="grid grid-cols-3 gap-2">
                             {positions.map(pos => (
                                 <button key={pos.name} onClick={() => handlePositionClick(pos.x, pos.y)} className="h-8 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg transition-colors shadow-sm" title={pos.name}></button>
                             ))}
                         </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-700">
                         {selectedOverlay.type === 'text' && selectedOverlay.templateId && (
                            <button onClick={onUpdateTemplate} className="text-sm bg-blue-900/30 border border-blue-500/40 hover:bg-blue-900/50 text-blue-300 font-black py-3 px-2 rounded-xl uppercase tracking-tighter transition-all">{t('updateTemplateButton')}</button>
                         )}
                         <button onClick={onDeleteOverlay} className="flex items-center justify-center gap-2.5 text-sm bg-red-900/30 border border-red-500/40 hover:bg-red-900/50 text-red-300 font-black py-3 px-2 rounded-xl col-span-2 uppercase tracking-tighter transition-all">
                             <TrashIcon className="w-5 h-5" /> {t('deleteLayerButton')}
                         </button>
                    </div>
                </div>
            )}
...
const ColorMixerPanel: React.FC<{
    ...
    const renderSlider = (channel: typeof COLOR_CHANNELS[number]) => (
        <div key={channel.id} className="grid grid-cols-6 items-center gap-3">
            <div className={`w-3.5 h-3.5 rounded-full ${channel.color} border border-white/20 shadow-md`}></div>
            <label onDoubleClick={() => onChange(channel.id, activeTab, 0)} className="text-sm text-gray-300 col-span-1 truncate cursor-pointer uppercase font-black tracking-tighter" title="Double click to reset">{t(channel.labelKey as any)}</label>
            <input
                type="range"
                min="-20" max="20" step="0.1"
                value={colorMixer[channel.id][activeTab]}
                onChange={e => onChange(channel.id, activeTab, Number(e.target.value))}
                className="w-full h-1.5 bg-gray-800 border border-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500 col-span-3"
            />
             <span onDoubleClick={() => onChange(channel.id, activeTab, 0)} className="text-sm text-gray-200 font-mono bg-gray-900 border border-white/5 px-2 py-0.5 rounded-md cursor-pointer text-center min-w-[40px] shadow-inner" title="Double click to reset">{colorMixer[channel.id][activeTab].toFixed(1)}</span>
        </div>
    );

    return (
        <div className="p-5 bg-gray-900/50 rounded-2xl border border-white/5 space-y-5 shadow-2xl">
            <div className="flex justify-between items-center">
                <h4 className="text-[15px] font-black text-gray-400 uppercase tracking-widest">{t('colorMixerLabel')}</h4>
                <button onClick={onReset} className="text-sm text-purple-400 hover:text-purple-300 font-black uppercase tracking-tight transition-colors">{t('resetButton')}</button>
            </div>
            <div className="flex bg-gray-800 rounded-xl p-1 border border-white/5 shadow-inner">
                <button onClick={() => setActiveTab('h')} className={`flex-1 py-2 text-sm font-black uppercase tracking-tighter rounded-lg transition-all ${activeTab === 'h' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-200'}`}>{t('hueLabel')}</button>
                <button onClick={() => setActiveTab('s')} className={`flex-1 py-2 text-sm font-black uppercase tracking-tighter rounded-lg transition-all ${activeTab === 's' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-200'}`}>{t('saturationLabel')}</button>
                <button onClick={() => setActiveTab('l')} className={`flex-1 py-2 text-sm font-black uppercase tracking-tighter rounded-lg transition-all ${activeTab === 'l' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-200'}`}>{t('luminanceLabel')}</button>
            </div>
