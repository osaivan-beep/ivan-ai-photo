        ...
        <div className="fixed inset-0 bg-gray-900/80 z-50 flex items-center justify-center p-2 sm:p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-gray-800 rounded-lg shadow-xl w-full h-full flex flex-col border border-gray-700" onClick={e => e.stopPropagation()}>
                ...
                <main className="flex-grow flex flex-col md:flex-row p-2 gap-2 overflow-hidden">
                    {/* Toolbar: Horizontal scroll on mobile, Vertical on Desktop */}
                    <div className="w-full md:w-48 flex-shrink-0 bg-gray-900/50 rounded-lg p-3 flex flex-row md:flex-col gap-3 overflow-x-auto md:overflow-visible items-center md:items-stretch scrollbar-thin">
                         <button onClick={() => fileInputRef.current?.click()} className="flex items-center justify-center gap-2 w-auto md:w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg flex-shrink-0 whitespace-nowrap">
                        ...
                        {selectedLayer && (
                            <div className="flex flex-row md:flex-col gap-2 pt-0 md:pt-2 border-l md:border-l-0 md:border-t border-gray-700 pl-3 md:pl-0 items-center md:items-stretch">
                                 <h4 className="text-sm font-semibold text-gray-400 text-center hidden md:block">Selected Layer</h4>
                                <button onClick={() => changeZIndex('up')} className="flex items-center gap-2 w-auto md:w-full bg-gray-700 hover:bg-gray-600 text-white py-2 px-3 rounded-lg flex-shrink-0 whitespace-nowrap"><BringForwardIcon className="w-5 h-5"/> <span className="hidden lg:inline">{t('layoutBringForward')}</span></button>
                                <button onClick={() => changeZIndex('down')} className="flex items-center gap-2 w-auto md:w-full bg-gray-700 hover:bg-gray-600 text-white py-2 px-3 rounded-lg flex-shrink-0 whitespace-nowrap"><SendBackwardIcon className="w-5 h-5"/> <span className="hidden lg:inline">{t('layoutSendBackward')}</span></button>
                                <button onClick={() => updateLayer(selectedLayerId!, { scaleX: selectedLayer.scaleX * -1 })} className="flex items-center gap-2 w-auto md:w-full bg-gray-700 hover:bg-gray-600 text-white py-2 px-3 rounded-lg flex-shrink-0 whitespace-nowrap"><MirrorIcon className="w-5 h-5"/> <span className="hidden lg:inline">{t('layoutMirror')}</span></button>
                                <button onClick={deleteLayer} className="flex items-center gap-2 w-auto md:w-full bg-red-600/80 hover:bg-red-700 text-white py-2 px-3 rounded-lg flex-shrink-0 whitespace-nowrap"><TrashIcon className="w-5 h-5"/> <span className="hidden lg:inline">{t('layoutDelete')}</span></button>
                            </div>
                        )}
                    </div>

                    {/* Canvas Area */}
                    <div ref={containerRef} className="flex-grow bg-gray-900/50 rounded-lg flex items-center justify-center p-2 overflow-hidden w-full">
                        <div
                            ref={canvasAreaRef}
                         ...
                         >
                            {layers.sort((a,b) => a.zIndex - b.zIndex).map(layer => (
                                <div
                                    ...
                                    onTouchStart={(e) => handleInteractionStart(e, 'move', layer)}
                                    style={{
                                        left: layer.x,
                                        top: layer.y,
                                        width: layer.width,
                                    ...
                                    {selectedLayerId === layer.id && (
                                        <>
                                            ...
                                            <div
                                                ...
                                                onTouchStart={(e) => handleInteractionStart(e, 'resize', layer)}
                                            />
                                            <div
                                                ...
                                                onTouchStart={(e) => handleInteractionStart(e, 'rotate', layer)}
                                            >
