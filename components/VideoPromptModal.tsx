...
export const VideoPromptModal: React.FC<VideoPromptModalProps> = ({ imageSrc, onClose, t, lang, userCredits, onDeductCredits }) => {
    ...
    const [camera, setCamera] = useState('vp_cam_auto');
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                alert(lang === 'zh' ? "生成失敗，請重試" : "Generation Failed, please try again");
            }
        } catch (e) {
            console.error(e);
                    ...
                    <div className="w-full lg:w-1/3 p-6 flex flex-col gap-6 overflow-y-auto border-r border-gray-800/50 bg-[#0f111a]">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                >
                                    {['vp_cam_auto', 'vp_cam_none', 'vp_cam_pan', 'vp_cam_tilt', 'vp_cam_zoom', 'vp_cam_orbit', 'vp_cam_dolly', 'vp_cam_fpv', 'vp_cam_follow', 'vp_cam_orbit_in', 'vp_cam_orbit_out'].map(c => (
                                        <option key={c} value={c}>{t(c as any)}</option>
                                    ))}
                                </select>
                            </div>

                            <div className="bg-[#1a1d2d] p-3 rounded-lg border border-gray-700">
                                <button 
                                    onClick={handleGenerate}
                                    disabled={loading}
                                    className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold py-3 px-6 rounded-lg transition-all transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                >
                                    {loading ? (
                    ...
                    <div className="w-full lg:w-2/3 bg-[#0a0b10] p-6 overflow-y-auto">
                        ...
                        {!loading && results.length === 0 && (
                            <div className="h-full flex flex-col items-center justify-center text-gray-500 opacity-50 min-h-[400px]">
                                ...
                                <p className="text-lg">{lang === 'zh' ? '等待生成...' : 'Ready to Generate'}</p>
                            </div>
                        )}
