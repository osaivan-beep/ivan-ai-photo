...
import { initializeFirebase, isFirebaseConfigured, login, register, logout, getUserProfile, deductCredits, addCreditsByEmail, getAuthInstance, sendPasswordReset, loginAsGuest } from './services/firebaseService';
    ...
    const handleAuthSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (errorCode === 'auth/email-already-in-use') {
                ...
                setError('找不到此帳號。');
            } else if (errorMessage.includes('blocked') || errorCode === 'auth/operation-not-allowed') {
                setError('登入失敗：請至 Firebase Console > Authentication 開啟「Email/Password」登入功能。');
            } else if (errorMessage.includes('api-key-not-valid')) {
                setError('系統設定錯誤：API Key 無效。請聯絡管理員。');
            } else {
             ...
             <div className="bg-gray-800 rounded-2xl shadow-xl w-full max-w-4xl border border-gray-700 overflow-hidden flex flex-col md:flex-row">
                <div className="bg-gradient-to-br from-purple-900 to-indigo-900 p-8 md:w-5/12 flex flex-col justify-center items-center text-center">
                    ...
                    <h1 className="text-3xl font-bold text-white mb-2">
                        ...
                        <span className="ml-2 text-xs align-top bg-green-600 text-white px-1.5 py-0.5 rounded border border-green-400 shadow-sm">v1.0.147</span>
                    </h1>
                    <p className="text-purple-200">{t('landingSubtitle' as any) || 'Professional AI Editor'}</p>
                </div>

                <div className="p-8 md:w-7/12 bg-gray-800 flex flex-col justify-center">
                    <form onSubmit={handleAuthSubmit} className="space-y-4 animate-fade-in">
                        <h2 className="text-2xl font-bold text-white mb-6 text-center">
                            {isLoginMode ? '歡迎回來' : '建立帳戶'}
                        </h2>
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-gray-900 text-white p-3 rounded border border-gray-600 focus:border-purple-500 focus:outline-none transition-colors"
                        ...
                        <button 
                            type="submit" 
                            disabled={loading}
                            className="w-full py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-lg transition-colors disabled:opacity-50 shadow-lg mt-4"
                        >
                            {loading ? '處理中...' : (isLoginMode ? t('loginButton') : '註冊')}
                        </button>
                        ...
                        <div className="text-center mt-4">
                            ...
                            >
                                {isLoginMode ? '還沒有帳號？立即註冊' : '已有帳號？登入'}
                            </button>
                        </div>
                    </form>
                </div>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Initialize Firebase automatically on load using embedded config
    try { 
        initializeFirebase(); 
  ...
  const handleApiError = (e: any) => {
      ...
      // More user friendly message for backend failures
      if (msg.includes('backend') || msg.includes('internal') || msg.includes('Startup Failed')) {
          msg = "系統連線錯誤 (Backend/Internal)。\n請確認：\n1. 您是否已綁定 Google Cloud Billing (Blaze 方案)。\n2. 您的 API Key 在 Google Cloud Console 中是否設定為「無限制」 (不要限制 IP)。\n3. (v1.0.147) 正在重試連線...";
      } else if (msg.includes('Missing API Key')) {
          msg = "後端錯誤：找不到 API Key (Secret Empty)。請重新執行 'firebase functions:secrets:set GEMINI_API_KEY'。";
      }
      setError(msg);
  };
        ...
        <header className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
             <div className="text-center md:text-left">
                <h1 className="text-4xl lg:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-500 to-red-500">
                    {t('title')} 
                    <span className="text-xs bg-green-600 text-white px-2 py-1 rounded border border-green-400 align-middle ml-2 font-bold shadow-sm animate-pulse">
                        v1.0.147
                    </span>
                </h1>
              ...
              {error && (
                <div className="p-4 bg-red-900/30 border border-red-500/50 rounded-xl text-red-200 text-sm">
                  <div className="flex justify-between items-start">
                   ...
                   {(error.includes('Rate Limit') || error.includes('429') || error.includes('系統忙碌') || error.includes('Backend') || error.includes('系統錯誤')) && (
